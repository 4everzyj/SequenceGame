...
const CARDS_BOARD_ARRAY = [
  ["W", "2S1", "3S1", "4S1", "5S1", "6S1", "7S1", "8S1", "9S1", "W"],
...
const PLAYER_WITH_SAME_NAME_ERROR =
  "Player with name '%s' is already in game. Choose different name";
...
function initializeGameState(gameRoomId, numPlayers) {
  let numTeams = getNumberOfTeamsForPlayers(numPlayers);
  let numSequencesForWin = getNumberOfSequencesForWin(numTeams);
  let GameState = {
...
function assignTeamColor(gameRoom, playerId) {
  let numPlayers = GAME_LOBBIES.get(gameRoom).numPlayers;
  let numTeams = getNumberOfTeamsForPlayers(numPlayers);
  let teamColorNum = playerId % numTeams;
  let teamColor = COLORS[teamColorNum];
  console.log("team color is " + teamColor);
  return teamColor;
}

function isDeadCard(gameRoom, card) {
  let GameState = GAME_LOBBIES.get(gameRoom);
  for (let deadCard of GameState.deadCards) {
    if (deadCard === card) {
      return true;
    }
  }
...
function getCard(gameRoom) {
  let card = GAME_LOBBIES.get(gameRoom).cardDeck.pop();
...
function createPlayer(gameRoom, id, name) {
  let team = assignTeamColor(gameRoom, id);
  ...
  let numCardsPerPerson = getNumberOfCardsForPlayers(
    GAME_LOBBIES.get(gameRoom).numPlayers
  );
  let player = {
    id: id,
    name: name,
...
function initializeNewGame(playerName, numPlayers, socketId) {
  let gameRoomId = generateRoomCode();
  let gameState = initializeGameState(gameRoomId, numPlayers);
  GAME_LOBBIES.set(gameRoomId, gameState);
  let player = createPlayer(gameRoomId, 0, playerName);
  // add new team
...
function isSequenceEqual(sequence1, sequence2) {
  for (let i = 0; i < SEQUENCE_LENGTH; i++) {
    let position1 = sequence1[i];
    let position2 = sequence2[i];
...
function checkOverlap(sequence1, sequence2) {
  for (let i = 0; i < SEQUENCE_LENGTH; i++) {
    for (let j = 0; j < SEQUENCE_LENGTH; j++) {
      let cell1 = sequence1.positions[i];
      let cell2 = sequence2.positions[j];
...
function addSequenceToTeam(gameRoom, team, sequence) {
  let GameState = GAME_LOBBIES.get(gameRoom);
  let teamSequences = GameState.teams.get(team).sequences;
...
function getPositionOfCardInHand(gameRoom, playerId, cardId) {
  ...
  let cardsInHand = GAME_LOBBIES.get(gameRoom).players.get(playerId)
    .cardsInHand;
  let wildCard = -1;
...
function checkDeadCards(gameRoom, card) {
  ...
  let cardClass = card.substring(0, card.length - 1);
  let cardPosition1 = CARD_POSITIONS[cardClass + "1"];
  let cardPosition2 = CARD_POSITIONS[cardClass + "2"];
  if (
    GameState.board[cardPosition1.x][cardPosition1.y].token != null &&
    GameState.board[cardPosition2.x][cardPosition2.y].token != null
  ) {
    // card is dead
...
function placeTokenOnCard(gameRoom, playerId, card) {
  ...
  let positionCard = getPositionOfCardInHand(gameRoom, playerId, card);
  if (positionCard === -1) {
  ...
  let position = CARD_POSITIONS[card];
  let colorToken = GameState.players.get(playerId).team;
  GameState.board[position.x][position.y].token = colorToken;

  // update last placed position
  GameState.lastPlayedPos = position;
  ...
  // check for sequences
  let sequences = getSequenceWithToken(
    gameRoom,
    colorToken,
    position.x,
    position.y
...
function updateNextPlayer(gameRoom) {
  let game = GAME_LOBBIES.get(gameRoom);
  let currentPlayer = game.currentPlayer;
  let length = game.players.size;
  let nextPlayer = currentPlayer >= length - 1 ? 0 : currentPlayer + 1;
...
function removeToken(gameRoom, playerId, card) {
  let GameState = GAME_LOBBIES.get(gameRoom);
  let positionOneEyeJack = getOneEyedJack(gameRoom, playerId);
  let isOccupied = checkIfCardOccupied(gameRoom, playerId, card);
  if (!isOccupied) {
    let err =
      "For gameRoom " +
      gameRoom +
      ...
      ", card " +
      card +
      " is not occupied or is occupied by the same team";
    throw err;
  }
  if (positionOneEyeJack === -1) {
    let err =
      ...
      " in game room " +
      gameRoom +
      " does not have a one eyed jack";
    throw err;
  }
  // get position of card and remove the token on it
  let position = CARD_POSITIONS[card];
  let isPartOfSequence = GameState.board[position.x][position.y].partOfSequence;
  if (isPartOfSequence) {
    let err = "Token is part of sequence";
  }
  ...
  GameState.players.get(playerId).cardsInHand[positionOneEyeJack] = getCard(
    gameRoom
  );
}
...
function addPlayerGameLobby(name, gameRoomId) {
  ...
  let gameRoom = GAME_LOBBIES.get(gameRoomId);
  if (!gameRoom) {
    throw INVALID_ROOM_CODE_ERROR;
  }
  let playerIds = Array.from(gameRoom.players.keys());
  let lastPlayer = gameRoom.players.get(playerIds[playerIds.length - 1]);
  let playerId = lastPlayer.id + 1;
  let numSameName = numElementInArr(name, gameRoom.playerNames);
  if (numSameName > 0) {
    throw PLAYER_WITH_SAME_NAME_ERROR.replace("%s", name);
  }
  let player = createPlayer(gameRoomId, playerId, name);
...
function removePlayerFromGame(gameRoomId, playerId) {
  ...
  let player = gameRoom.players.get(playerId);
  if (!player) {
    return;
  }
  ...
  for (let i = 0; i < gameRoom.teams.get(player.team).players.length; i++) {
    let teamPlayer = gameRoom.teams.get(player.team).players[i];
  ...
  let playerNames = gameRoom.playerNames;
...
io.on('connection', function(socket) {
  ...
  socket.on('disconnect', function() {
    ...
    let gamePlayer = SOCKETS_MAP.get(socket.id);
    if (gamePlayer) {
      // remove player from game
      console.log(
        "Player in gameRoom " +
          gamePlayer.gameRoomId +
          " and id " +
          gamePlayer.playerId +
          " is leaving game"
      );
  ...
  socket.on("createGame", function (msg) {
    ...
    console.log(msg);
    let gameRoomId = initializeNewGame(msg.name, msg.numPlayers, socket.id);
...
function sendStateUpdate(gameRoomId) {
  ...
  if (gameRoom) {
    ...
    let players = new Map(gameRoom.players);
    let teams = new Map(gameRoom.teams);
